...
def celebrity(M):
    n = len(M)
    # Create a stack and push all people into it
    st = list(range(n))
    ...
    while len(st) > 1:
        ...
        # If a knows b, a cannot be a celebrity, so push b back
        # If a doesn't know b, b cannot be a celebrity, so push a back
        if M[a][b] == 1:
            st.append(b)
        else:
            st.append(a)
...
for t in test:
    result = celebrity(t)
    if result != -1:
        print(f"Using Stack: {result}")
    else:
        print("No celebrity in the party")
